...
const HOLE_ALPHA_THRESHOLD = 0.4 # Alpha value below which a pixel is considered transparent
...
func _detect_hole(img: Image) -> Rect2:
	...
	# Start scan points
	var seed_x = int(w / 2.0)
	var seed_y = int(h / 2.0)
	
	# 1. SEARCH FOR SEED PIXEL
	# Check center first
	if img.get_pixel(seed_x, seed_y).a > HOLE_ALPHA_THRESHOLD:
		var found = false
		
		# Scan vertical center line (common for portrait bezels with offset display)
		for y in range(0, h, 8):
			if img.get_pixel(seed_x, y).a <= HOLE_ALPHA_THRESHOLD:
				seed_y = y
				found = true
				break
		...
		if not found:
			for x in range(0, w, 8):
				if img.get_pixel(x, seed_y).a <= HOLE_ALPHA_THRESHOLD:
					seed_x = x
					found = true
					break
					
		# Full grid scan (last resort)
		if not found:
			for y in range(0, h, 16):
				for x in range(0, w, 16):
					if img.get_pixel(x, y).a <= HOLE_ALPHA_THRESHOLD:
						seed_x = x
						seed_y = y
						found = true
						break
		...
		if not found:
			print("BezelOverlay: No transparent hole found! Fallback to full image.")
			return Rect2(0, 0, w, h)

	# 2. REFINE AND EXPAND
	# We have a seed. Now expand to find the bounds.
	# To handle non-perfectly-rectangular or non-centered seeds, we refine twice.
	
	# Horizontal scan from initial seed
	var right = seed_x
	while right < w and img.get_pixel(right, seed_y).a <= HOLE_ALPHA_THRESHOLD:
		right += 1
	var left = seed_x
	while left >= 0 and img.get_pixel(left, seed_y).a <= HOLE_ALPHA_THRESHOLD:
		left -= 1
		
	# Move to horizontal center of discovered segment
	seed_x = int((left + right) / 2.0)
	
	# Vertical scan from new horizontal center
	var bottom = seed_y
	while bottom < h and img.get_pixel(seed_x, bottom).a <= HOLE_ALPHA_THRESHOLD:
		bottom += 1
	var top = seed_y
	while top >= 0 and img.get_pixel(seed_x, top).a <= HOLE_ALPHA_THRESHOLD:
		top -= 1
		
	# Final seed Y at vertical center
	seed_y = int((top + bottom) / 2.0)
	
	# Final horizontal scan at vertical center (important for circles/curved shapes)
	right = seed_x
	while right < w and img.get_pixel(right, seed_y).a <= HOLE_ALPHA_THRESHOLD:
		right += 1
	left = seed_x
	...
	# Sanity check: if hole is implausibly small, it might be a glitch
	if hole_w < 10 or hole_h < 10:
		print("BezelOverlay: Detected hole too small (", hole_w, "x", hole_h, "), ignoring.")
		return Rect2(0, 0, w, h)
		
	var r = Rect2(hole_x, hole_y, hole_w, hole_h)
	print("BezelOverlay: Detected hole found: ", r)
	return r
